...
// GET /api/categories/expense?budgetId=xxx
export async function GET(request: Request) {
  try {
    ...
    const categoriesCollection =
      await getCollection<ExpenseCategory>("expense_categories");
...
// POST /api/categories/expense - Create expense category
export async function POST(request: Request) {
  try {
    ...
    const newCategory: Omit<ExpenseCategory, "_id"> = {
    ...
    const result = await categoriesCollection.insertOne(
      newCategory as ExpenseCategory
    );
